<?php
/**
 * TTBase Recent Posts Grid Widget
 */
...
// Start class
class TTBase_Framework_Recent_Posts_Grid_Widget extends WP_Widget {
	private $defaults;
	...
	public function __construct() {
		...
		parent::__construct(FALSE, $name = esc_html__('TTBase Recent Posts Grid', 'ttbase-framework'), array(
				'description' => esc_html__('Show the latest posts in a grid style.', 'ttbase-framework')
			));

		$this->defaults = array(
			'title'      => esc_html__( 'Recent Posts', 'ttbase-framework' ),
			'number'     => '6',
			'post_type'  => 'post',
			'taxonomy'   => '',
			'terms'      => '',
		...
		<ul class="recent-posts-grid-widget grid-row clearfix">
			...
			while ( $ttbase_query->have_posts() ) : $ttbase_query->the_post();
				...
				<li class="<?php echo gymx_grid_class( $columns ); ?> nr-col col-<?php echo esc_attr($count); ?>">
					<a href="<?php get_permalink(get_the_ID()); ?>" title="<?php gymx_esc_title(); ?>">
						<?php the_post_thumbnail( 'gymx-img-size-mini' ); ?>
						<div class="overlay">
...
if ( ! function_exists( 'ttbase_framework_register_recent_posts_grid_widget' ) ) {
	function ttbase_framework_register_recent_posts_grid_widget() {
		register_widget( 'TTBase_Framework_Recent_Posts_Grid_Widget' );
	}
}
add_action( 'widgets_init', 'ttbase_framework_register_recent_posts_grid_widget' );