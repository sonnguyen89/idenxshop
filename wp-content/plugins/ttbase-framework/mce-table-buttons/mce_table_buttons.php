<?php
/**
 * MCE Table Buttons Plugin developed by Jake Goldman, 10up, Oomph 
...
class MCE_Table_Buttons {
	...
	public static function mce_external_plugins( $plugin_array ) {
		...
		if ( version_compare( $tinymce_version, '400', '<' ) ) {
			...
			wp_register_style( 'mce-table-buttons', TTBASE_FRAMEWORK_URL . 'mce-table-buttons/tinymce3-assets/mce-table-buttons.css' );
			wp_print_styles( 'mce-table-buttons' );

			$plugin_dir_url = TTBASE_FRAMEWORK_URL;
			$plugin_array['table'] = $plugin_dir_url . 'mce-table-buttons/tinymce3-table/editor_plugin.js';
			$plugin_array['mcetablebuttons'] = $plugin_dir_url . 'mce-table-buttons/tinymce3-assets/mce-table-buttons.js';

		} elseif ( version_compare( $tinymce_version, '4100', '<' ) ) {

			$plugin_array['table'] = TTBASE_FRAMEWORK_URL . 'mce-table-buttons/tinymce4-table/plugin' . $variant . '.js';

		} else {

			$plugin_array['table'] = TTBASE_FRAMEWORK_URL . 'mce-table-buttons/tinymce41-table/plugin' . $variant . '.js';
