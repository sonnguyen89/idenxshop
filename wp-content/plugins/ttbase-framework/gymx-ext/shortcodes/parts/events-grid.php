...
// Event Grid -------------------------------------------------------------------------- >
function ttbase_framework_event_grid_shortcode($atts) {
	...
	$args = array(
		'post_type' => $timetable_post_type,
		'posts_per_page' => intval( $events ),
		'order'          => $order,
		'orderby'        => $orderby,
		'post_status'    => 'publish',
	...
	if($showfilter == 'yes'){ 
		...
		if($categories != '' && $categories != 'all') {
			...
			$class_filters = get_terms('events_category');
			$arraytostring = '';
			...
			<?php if($class_filters): ?>
    			<ul class="ttbase-class-filter-list">
    				...
    				<?php foreach($class_filters as $class_filter => $value){ ?>
    					<?php $filter_name = get_term_by('slug',$value,'events_category'); ?>
	...
	$event_output = ob_get_contents(); // get buffered content

	ob_end_clean(); // clean buffer

	return $event_output;
}
add_shortcode("ttbase_event_grid", "ttbase_framework_event_grid_shortcode");
